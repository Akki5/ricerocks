...
class Ship:
    ...
    def shoot(self):
        ...
        missile = Sprite(missile_pos,missile_vel,0,0,missile_image,missile_info)
...
def new_game():
    global lives,score,started,ship,rock_group,missile_group
    lives = 3
    score = 0
    started = True
...
def process_sprite_group(sprite_group,canvas):
    ...
    discard(temp_group,sprite_group)
    for element in sprite_group:
         element.draw(canvas)
            
# helper function for collide functions 
def discard(sub_group,group):
    ...
    if started:
        ...
        for element in group:
            if element.collide(other_object):
                temp_group.append(element)
                lives-=1
        discard(temp_group,group) 
        if lives<1:
            started = False

# collision of missiles with asteroids    
def group_group_collide(first_group, second_group):
    global score
    if started:
        temp_group1 = []
        temp_group2 = []
        for element1 in first_group:
            for element2 in second_group:
                if element2.collide(element1):
                    temp_group1.append(element1)
                    temp_group2.append(element2)
                    score+=1
        discard(temp_group1,first_group)
        discard(temp_group2,second_group)
